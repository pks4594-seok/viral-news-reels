...
class _ReelDetailScreenState extends State<ReelDetailScreen>
  ...
  /// 플레이어 컨트롤러 — 자막 타임라인과 양방향 연동합니다.
  /// 릴스가 렌더링 완료된 시점에 생성하고, 화면 종료 시 해제합니다.
  ReelPlayerController? _player;
  String? _boundReelId;
  ...
  /// 릴스가 준비되면 컨트롤러를 연결 (한 번만)
  void _bind(ReelProject reel) {
    if (!reel.isReady) return;
    if (_boundReelId == reel.id && _player != null) return;
    _player?.dispose();
    _player = ReelPlayerController(reel: reel, vsync: this);
    _boundReelId = reel.id;
    WidgetsBinding.instance.addPostFrameCallback((_) => _player?.play());
  }
      ...
      body: ListView(
        ...
        children: [
          ReelPlayerView(
            reel: reel,
            controller: _player,
            onFullscreen: reel.isReady
                ? () => _openFullscreen(context, state, reel)
                : null,
          ),
  ...
  /// 전체 화면 재생으로 이동 — 준비된 릴스 목록을 함께 넘겨 스와이프 가능하게
  void _openFullscreen(
      BuildContext context, AppState state, ReelProject reel) {
    _player?.pause();
    final list = state.readyReels.isNotEmpty ? state.readyReels : [reel];
    var idx = list.indexWhere((r) => r.id == reel.id);
    if (idx < 0) idx = 0;
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => ReelPlaybackScreen(reels: list, initialIndex: idx),
      ),
    );
  }
...
/// 자막 타임라인 — 재생 위치를 실시간으로 따라가며 현재 자막을 강조합니다.
/// 항목을 탭하면 해당 자막 시점으로 탐색합니다.
class _TimelineSection extends StatelessWidget {
    ...
    return Column(
      ...
      children: [
        SectionHeader(
          title: '자막 스크립트',
          subtitle: ctrl != null
              ? '${reel.captions.length}개 컷 · 탭하면 해당 시점으로 이동'
              : '${reel.captions.length}개 컷',
          icon: Icons.closed_caption_rounded,
          accent: AppColors.neonCyan,
        ),
        const SizedBox(height: 12),
        if (ctrl == null)
          ...reel.captions.asMap().entries.map(
                (e) => _row(context, e.key, e.value, false, null),
              )
        else
          AnimatedBuilder(
            animation: ctrl,
            builder: (context, _) {
              final active = ctrl.captionIndex;
              return Column(
                children: reel.captions
                    .asMap()
                    .entries
                    .map((e) => _row(
                          context,
  ...
  Widget _row(BuildContext context, int i, CaptionLine c, bool active,
      VoidCallback? onTap) {
    return Padding(
